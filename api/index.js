...
// Helper to parse XML rates (matching the logic in mobile/web contexts)
...
const processModifiedRates = (xmlText, settings, existingData) => {
    ...
    // Modification logic matched with frontend
    ...
    Object.keys(rawData).forEach(id => {
        ...
        const isGold = id === '945'; // Gold 999
        const isSilver = ['2966', '2987'].includes(id); // Silver RTGS items
...
// Routes
// Updated: Get persisted live rates from MongoDB with High/Low calculations
app.get('/api/rates/live', async (req, res) => {
    ...
    try {
        ...
        if (!settings) settings = { baseModifications: { gold999: { value: 0 }, silver999: { value: 0 } } };
        ...
        if (isStale) {
            try {
                ...
                console.error("Rate Processing Error:", fetchErr.message);
            }
        }

        if (!rateDoc) return res.status(404).send('No rates found in database');

        // Support ID-specific response format as requested { rate, high, low }
        ...
        if (symbolId) {
            ...
            return res.status(404).json({ message: 'Symbol not found in calculated rates' });
        }

        // Default response: keep 'text' for backward compatibility + add new structured 'rates'
        res.json({ 
