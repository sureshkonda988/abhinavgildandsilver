...
app.post('/api/music/library/upload', (req, res) => {
    upload.single('file')(req, res, async (err) => {
        ...
        try {
            ...
            fs.writeFileSync(path.join(musicDir, filename), req.file.buffer);
            const fileUrl = `/music/${filename}`;

            let newTrack = { _id: Date.now().toString(), title, filename, url: fileUrl, size: req.file.size, createdAt: new Date() };
